package com.demo.core.animation {
	...
	public class BitmapAnimation extends Bitmap {
		...
		public var renderBySelf : Boolean;
		/**
		 * 多少毫秒切一张图
		 */
		...
		private function addSelfRender() : void {
		...
		protected function gotoAndSet(frame : int, adjustPosition : Boolean = true) : void {
			...
			// old version
			// if (flipH) {
			// if (bitmapData != currentFramData.reverseBitmapData) {
			// bitmapData = currentFramData.reverseBitmapData;
			// }
			// } else {
		...
		public function updateBrush():void
		{
			clearBrushs();
			if(currentFramData && currentFramData.buildBrushs && this.parent)
			{
				for each(var brush:BuildBrush in currentFramData.buildBrushs)
				{
					if(!this.parent.contains(brush))
						this.parent.addChild(brush);
				}
			}
		}
		private function clearBrushs():void
		{
			if(currentFramData && currentFramData.buildBrushs)
			{
				for each(var brush:BuildBrush in currentFramData.buildBrushs)
				{
					if(null != brush.parent)
		...
		public function get yOffset() : Number {
			if (currentFramData && currentFramData.offsetY != 0)
				return currentFramData.offsetY;
			return _yOffset;
		}
	}
}