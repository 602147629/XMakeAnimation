package com.demo.core.animation {
	...
	public class BitmapFrameData {
		private var _buildBrushs:Dictionary = new Dictionary();
		/**
		 * 这一帧上的代码
		 */
		...
		public function addBuildBrush(brush:BuildBrush):void
		{
			if(buildBrushs)
			{
				if(buildBrushs[brush.name])
					return;
				buildBrushs[brush.name] = brush;
			}
		}
		
		public function removeBuildBrush(name:String):void
		{
			buildBrushs[name] = null;
			 delete buildBrushs[name];
		}
		public function getBrushById(id:String):BuildBrush
		{
			if(null == buildBrushs)
				return null;
			return buildBrushs[id];
		}
		/**
		 * 鼠标响应区域
		 */
		...
		public function get buildBrushs():Dictionary
		{
			return _buildBrushs;
		}
